...
class topshop_ctl_trade_detail extends topshop_controller{
    public function index()
    {
        ...
        if(!empty($tradeInfo['tid'])){
            //获取报文信息
            $pagedata['mess']=kernel::single('sysshop_xml_message')->get_mess($tradeInfo['tid']);
        }
        //获取默认图片信息

        $cddata['name']=$tradeInfo['receiver_name'];
        $cddata['card_id']=$tradeInfo['card_id'];

        //身份证查询
        $iscard = app::get('topc')->rpcCall('user.card.index', $cddata);
        if($iscard){
            $tradeInfo['card_id'] = $iscard;
        }
    ...
    public function make_mess(){

        $postData = input::get();
        $params['tid']=$postData['tid'];
        $params['type']=$postData['type'];
        $res=kernel::single('sysshop_xml_message')->get_message($params);

        if($res[0]==0){
            $data=['error'=>1,'message'=>$res[1]];
        }else{
            $data=['error'=>0];
        }
