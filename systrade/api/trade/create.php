...
class systrade_api_trade_create{
    ...
    public function createTrade($params)
    {
        ...
        $data['trade_memo'] = $params['mark'];  		//订单备注信息；分店铺下面再分（业务类型+仓库地址）
        $data['invoice'] = array(
        ...
        if($params['addr_id'])
        {
            ...
            $data['delivery'] = array(
				...
				'card_id' => isset($addr['card_id']) ? $addr['card_id'] : "",			// 身份证号码 2016/4/6雷成德
        ...
        if($params['shipping'])
        {
            ...
            foreach($arr1 as $value)
            {
                $arr2 = explode(':',$value);
                list($key,$val) = $arr2;
                $data['shipping'][$key]['template_id'] = $val; // $key 是店铺ID
            }
        }
        else
        {
