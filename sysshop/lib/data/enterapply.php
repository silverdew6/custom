        ...
        if($postdata['enterapply_id'])
        {
            ...
            if($logs['enterlog']){
                ...
                $postdata['enterlog'] = array_merge($log,$postdata['enterlog']);
					...
					//重新写入商家申请表里面,后台系统修改商铺跨境信息
					if($postdata['update']==1){
			$postdata['tax']=$postdata['shop_info']['tax'];
			$postdata['sea_region']=$postdata['shop_info']['sea_region'];
			$postdata['shortName']=$postdata['shop_info']['shortName'];
			$postdata['agentName']=$postdata['shop_info']['agentName'];
			$postdata['agentCode']=$postdata['shop_info']['agentCode'];
			$postdata['ebcName']=$postdata['shop_info']['ebcName'];
			$postdata['ebcCode']=$postdata['shop_info']['ebcCode'];
					                  }
