   ...
   /**
    ...
    public function generate($tradeParams, $aCart=array() )
    {
        ...
        try
        {
        	 ...
        	 * 		)
        	 *      WAP debug throw new \LogicException(print_r($tradeData[1][12],true)); 
        	 * 		print_r($childlist); var_dump($result); $db->commit();exit;
        	 * 		Debug print_r($save_shopTax); $db->commit();exit; 
        	 * 		WAP_debug $db->commit(); throw new \LogicException(print_r($save_shopTax,true)); exit;
        	 */
            $tradeData = $this->_chgdata_new($tradeParams, $aCart); // $this->_chgdata($tradeParams, $aCart); 
		    ...
		    */
            ...
            foreach($tradeData as $shopId=>$firstShopOrder) {  
            	...
            	if($firstShopOrder){
            		foreach($firstShopOrder as $region_idd => $childlist){ //子订单
            			...
            			if($ttid = $childlist["tid"]){
            				$s = $this->__updatePromotionLog($childlist,$ttid,userAuth::id());
            			}
            		}
                ...
                if(!$result){
                	...
                	$this -> __clearSucessCart($login_user_Id , $shopId ,$staxList );	//清空购物车信息
				}
		     } //###一个店铺订单结束
		     
             $this->_couponUse($tradeData);	//将已使用的优惠券更新为已使用
             $db->commit();
        }
        catch (Exception $e)
        {
        ...
        $this->__unsetCartUseCoupon(); //清空保存在Session中购物车中已使用优惠券码列表；
     ...
     *
     */
    public function elsectroniCorder(){
     ...
     * @return $tid
     */
    protected function genId($userId, $isTid=true,$tax=0 )
    {
        if( ! $this->genidData )
        {
        ...
        $id = $this->genidData['tax'].$this->genidData['tradeBaseTime'].str_pad(++$this->genidData['tradeBaseRandNum'],2,'0',STR_PAD_LEFT).$this->genidData['tradeModUserId'];
    ...
    {
    	//发票信息；
    	$invoiceData = isset($tradeParams["invoice"]) ? $tradeParams["invoice"] : false ;
        return [
            'user_id'           => $tradeParams['user_id'],
            'user_name'         => $tradeParams['user_name'],
            ...
            'need_invoice'      => ($invoiceData['need_invoice'] ? 1 : 0),
            'trade_from'        => $tradeParams['source_from'],
            'invoice_name'      => $invoiceData['invoice_title'],
            'invoice_main'      => strip_tags($invoiceData['invoice_content']),
    ...
    function init_shopOrderData($shopCartData ,$templateId=false, $userId = 0,$otherParams = array()){
        ...
        if(isset($shopCartData["object2"]) && !empty($shopCartData["object2"])){
        	...
        	foreach($shopCartData["object2"] as $shop_region => $order){                                                //循环每个仓库的订单的订单数据
        		$sea_region_id = 0;
        		...
        		$youxiao_o_ratemoney = 0;   // 子订单综合税金额
        		...
        		if($shop_region){ //店铺ID + 仓库ID， 组合键
		            ...
		            //新订单号生成
	        		if($userId){
	        		  $t_tid = $this -> genId($userId,true,$tax); //有用户就创建订单号；
	        		}

                    //17/08/04
                  /*  $str_t=substr(trim($t_tid), 0, 1 );                                                          //截取字符串第一位数字
                    if($str_t!=(string)$tax){
                        $t_tid=substr_replace($t_tid,(string)$tax,0,1);
                    }*/
                    //17/08/04
            		...
            		if(isset($chailds) && !empty($chailds) && count($chailds) > 0 ){
            			...
            			/**--------------------------------------------by xch ------------------------------------------***/
	            			...
	            			if($order_tax_index >= 2 && $zonghe_rate_value > 0){
	            				$youxiao_o_ratemoney = ecmath::number_multiple(array($youxiao_o_paymoney,$zonghe_rate_value ));
	            				$returnShopData[$shop_region]["zonghe_ratemoney"]= $youxiao_o_ratemoney; //记录子订单的综合税；
	            				//子订单实付 = 订单额  + 收税额
	            			...
	            			$all_order_ratemoney  = ecmath::number_plus(array($all_order_ratemoney , $youxiao_o_ratemoney)) ; 	//计总税金额
        		...
        		$baseShopData["shop_total"]=array(
        			...
        			"all_rate_price" => isset($all_order_ratemoney) ? $all_order_ratemoney : 0  //总计税金额
        		);
        		
        		//当前订单使用的店铺优惠券 用于记录使用店铺优惠券日志（ 有问题，后续再修改改）
	            if(isset($_SESSION['cart_use_coupon'][$this->userIdent])  &&  isset($_SESSION['cart_use_coupon'][$this->userIdent][$shopId])) {
	            	$coupon_codeSn = $_SESSION['cart_use_coupon'][$this->userIdent][$shopId];
	            	//var_dump($coupon_codeSn);
	                if($coupon_codeSn){
	                	$this->cartUseCoupon[$shopId] = $coupon_codeSn; //保存用户当前订单使用的优惠券码；
	                	$baseShopData["shop_total"]["use_copon_sn"]= $coupon_codeSn;
	                	//通过优惠码计算优惠金额；
	                	$ciinfo =  kernel::single('syspromotion_coupon')->getCouponInfoByCode($coupon_codeSn);
	                	if($ciinfo && $ciinfo["valid"]==1 && isset($ciinfo["deduct_money"]) && floatval($ciinfo["deduct_money"]) >0 ){
	                		$baseShopData["shop_total"]["coupon_discount_money"] = floatval($ciinfo["deduct_money"]) ;
	                	}
	                }
	            }
    ...
    function _chgdata_new( $tradeParams, $aCart=array()){
        ...
        //print_r($aCart);exit;
    	$ziti_address = "";
    	foreach ($aCart['resultCartData'] as $shopId => $tval) {                                                        //循环店铺信息
    	 	$this->shopIds[] = $shopId;
    	 	//取出商品参数过的活动信息；
            ...
            //单个 店铺订单数据结构排序
        	$shop_orderinfo  = $this->init_shopOrderData($tval,$tempId,$tradeParams['user_id'], $otherparam);            //单店铺数据信息
        	//其它组合数据
            ...
            //新增了一个优惠券抵扣  //然后在子订单中寻找一个订单把优惠金额挂进去就可以；
            $useCouponMoney = 0 ;
            if(isset($shop_orderinfo["shop_total"]) && isset($shop_orderinfo["shop_total"]["coupon_discount_money"])){
            	$useCouponMoney = floatval($shop_orderinfo["shop_total"]["coupon_discount_money"]);
            }
        	...
        	if($shop_orderinfo && !empty($shop_orderinfo["orderlist"])){
        		foreach($shop_orderinfo["orderlist"] as $sr_id => $child_od){
        			...
        			if($child_od && floatval($child_od["payment"])>0){
    	 				...
    	 				/*订单支付金额   ＝ （ 订单实际金额   - 优惠券抵扣  ） + 应交税款   by xch  */
    	 				if($is_fenpei_coupon === 1 && $useCouponMoney > 0 ){ 		//挂在当前订单的抵扣优惠券金额；==可以，挂成功
    	 					$realPayM = ecmath::number_minus(array($child_od["payment"] , $useCouponMoney ));
    	 					if($realPayM > 0){      //==可以挂 ＝＝
    	 						$child_od["use_coupon_money"] = $useCouponMoney; //标识当前订单已使用优惠券抵扣了一些金额；
    	 						$is_fenpei_coupon = 0;
    	 					}else{
    	 						$realPayM = $child_od["payment"];
    	 						$is_fenpei_arr[] = $child_od["payment"];
    	 					}
    	 					$child_od["payment"] = $realPayM ;//订单支付金额 
    	 				}
    	 				//最后把税给带上；
        				...
        				$child_orderInfo = array_merge($shopTradeData , $child_od);	//一个子订单结构；
        				$sea_reginOderlist [$sr_id]= $child_orderInfo;
        			}
        		}
        		
        		//最后再整理一下优惠券是否被挂上去；
        		if($is_fenpei_coupon === 1  && !empty($is_fenpei_arr) && $useCouponMoney>0 ){
        			$number_hee = ecmath::number_plus($is_fenpei_arr);
        			$child_nums = count($is_fenpei_arr);
        			if($number_hee >0){
        				$child_nums_index = 1;
        				$fenpeiMoney_arr = array();
        				foreach($sea_reginOderlist as $rs_id => $vks){
	        				//子订单中都分配这个优惠券额；
	        				$last_ordermoney = $sea_reginOderlist[$rs_id]["payment"];
	        				//前 N－1 个采用计算，最后一个用减法
	        				if($child_nums_index < $child_nums){
	        					$fenpeiMoney = number_format($useCouponMoney *($vks["payment"] / $number_hee),2,'.','');
	        					$fenpeiMoney_arr[] =$fenpeiMoney; 
	        					$last_ordermoney = ecmath::number_minus(array($last_ordermoney, $fenpeiMoney ));//挂分配减少的订单金额
	        				}else{
	        					if($fenpeiMoney_arr && !empty($fenpeiMoney_arr)) {
	        						$number_hee2 = ecmath::number_plus($fenpeiMoney_arr); //已分配了N－1个出去的总和
	        						$fenpeiMoney = ecmath::number_minus(array($useCouponMoney, $number_hee2 ));
	        						$last_ordermoney = ecmath::number_minus(array($last_ordermoney, $fenpeiMoney ));
	        					}
	        				}
	        				//当前计算的订单，已经把税都计算在内了。
	        				$sea_reginOderlist[$rs_id]["payment"] = $last_ordermoney > 0?$last_ordermoney :0;
	        				$sea_reginOderlist[$rs_id]["use_coupon_money"] = $fenpeiMoney>0 ?$fenpeiMoney :0 ; //标识当前订单已使用优惠券抵扣了一些金额；
	        				$child_nums_index ++;
	        			}
        			}
        		}
        	}
        	//throw new \LogicException(print_r($sea_reginOderlist,true)); 
        	//把各个仓库的订单加到店
        ...
        foreach ($aCart['resultCartData'] as $shopId => $shopCartData )
        {
            ...
            if(isset($shopCartData["object2"]) && !empty($shopCartData["object2"])){
            	foreach($shopCartData["object2"] as $shop_region => $order){
            		...
            		$chailds 	= 	array();		  //子订单列表
            		//新订单号生成
	            	$tid = $this->genId($tradeParams['user_id'],true,$order_tax_index);

                    //17/08/04
                    /*$str_t=substr(trim($tid), 0, 1 );                                                          //截取字符串第一位数字
                    if($str_t!=(string)$order_tax_index){
                        $tid=substr_replace($tid,(string)$order_tax_index,0,1);
                    }*/
                    //17/08/04
     ...
     * 因交易数据结构修改了，把$tradeData里的订单号修改成多个子订单的订单号的集合；用逗号隔开。
     * 会考虑优惠券平摊的情况，所以也tid 字段也修改为 text数据类型；
     */
    protected function _couponUse($tradeData)
    {
        foreach((array)$this->cartUseCoupon as $shopId => $couponCode) {
        	$tid_arrObj = $tradeData[$shopId];
        	$tid_arr = array();
        	$tid_target = $tradeData[$shopId]['tid']; //默认取原结构ID；
        	$tids_list =""; //目录订单列表信息；
        	if($tid_arrObj && !empty($tid_arrObj)){
        		foreach($tid_arrObj as $tssd){
        			isset($tssd["tid"]) and $tid_arr [] = $tssd["tid"];
        		}
        		if($tid_arr){
        			$tid_target = reset($tid_arr); 		//取第一个订单号
        			$tids_list = implode(",",$tid_arr); //  新结构的订单号==多个ID组合
        		}
        	}
        	if($tid_target && $couponCode){
        		$data = array('tid' => $tid_target, 'coupon_code' => $couponCode ,'use_tids' => $tids_list);
