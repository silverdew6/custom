...
class sysuser_passport
{
    ...
    private function __preAccountUser($data)
    {
        ...
        $account['openid'] = $data['openid'];  //2015/12/19雷成德微信增加登录ID
    ...
    private function __preUser($userId, $data)
    {
		...
		$user['usercard'] = trim($data['usercard']); //身份证添加2016/4/24  雷成德
		$user['username'] = trim($data['username']); //身份证添加2016/4/24  雷成德
        $user['regtime'] = time();
        return $user;
    }
    ...
    public function memInfo($userId)
    {
        if( !$this->memInfo[$userId] )
        {
			...
			$mobile= (isset($account["mobile"]) && !empty($account["mobile"])) ? $account["mobile"]: false;
			$mobile_qrcimg = "";//手机号二维码图片
			if($mobile){ 
				//手机号生成二维码图片  ___ by XCH (2016年9月19号)；
				$couponQrcMdl  = kernel::single('topm_couponqrc');
				$couponQrcMdl->setrootDir("mobilepng");
				$fileMd5 = md5($mobile ."JM") . substr($mobile,-4); //加密手机号文件
				//判断当前手机号是否已经有生成二维码了；没有就生成
				if(!$couponQrcMdl->mobilQrcExists($fileMd5,$mobile_qrcimg )){
					 $cfile_qrcc = $couponQrcMdl -> createQrc(array($mobile,$fileMd5),$mobile_qrcimg,true); 	
				}
			}
            $memInfo = ['userId' => $userId,
                        ...
                        'sex' => $sysUserInfo['sex'],
				        'usercard' => $sysUserInfo['usercard'], //查询用户的身份证号码 2016/4/24雷成德
        	...
        	if(!isset($http_host) || empty($http_host)) { $http_host = "http://www.myjmall.com"; }
            $mobile_qrcimg and $memInfo["mobile_qrc"] = $http_host. "/". $mobile_qrcimg;
