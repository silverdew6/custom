...
class syscategory_ctl_admin_cat extends desktop_controller {
    ...
    public function save()
    {
        ...
        {
        	// 添加分类的简写和 全拼
        	$catName = isset($postData['cat']['cat_name']) ? trim($postData['cat']['cat_name']) : '';
        	if($catName) {
        		include("pinyin.php");
        		$postData['cat']['name_jp'] = pinyin($catName,"first","",""); // first 字母 
        		$postData['cat']['name_quanp'] = str_replace(" ",'',pinyin($catName,"all","","")); // all  全拼 ,并去掉空格
        		//临时更新  更新所有数据； by xch 2017年 2月11号 ；更新一次就关闭
        		/*$alldata = $objMdlCat->getList('*',null,0,1000);
        		foreach($alldata as $k=> $vd){
        			$ccid = isset($vd["cat_id"]) ? intval($vd["cat_id"]) : 0;
        			if($ccid && trim($vd["cat_name"]) !=""){
        				$vd['name_jp'] = pinyin($vd["cat_name"],"first","",""); // first 字母 
        				$vd['name_quanp'] = str_replace(" ",'',pinyin($vd["cat_name"],"all","","")); // all  全拼 ,并去掉空格
        				$objMdlCat->save($vd);
        			}
        		}*/
        	}
