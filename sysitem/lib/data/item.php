...
class sysitem_data_item {
    ...
    function add($postData)
    {
        ...
        $objMdlCat = app::get('syscategory')->model('cat');
        $item = $this->_prepareItemData($postData);//商品参数
        $select_tax_type = isset($item['tax']) ? intval($item['tax']) : 0; 
        if($postData['item']['item_id']){  //编辑状态下；
            $params =array(
        ...
        foreach($item['sku'] as $k => $p)
        {
            ...
            $isEdit = isset($item['item_id']) && intval($item['item_id'])>0 ? true :false; //是否编辑状态
            if(!$isEdit && $this->__checkProductBn($p['bn'], $p['sku_id'], $item['shop_id'], 'sku')){
		...
		//雷成德 2016/5/18 添加税率  by xch 09.08
		if(isset($select_tax_type) && in_array($select_tax_type,array(2,3) )){ //保存商品的行邮税
			$item['gcode']= trim($item['gcode'])!="" ? trim($item['gcode']) : "" ; 			 //商品编码
			$item['postyun_code']= trim($item['postyun_code'])!="" ? trim($item['postyun_code']):"0";
			$item['postyun_rate']= isset($item['postyun_rate']) ? floatval($item['postyun_rate']):0;
			$item['tax_rate']= isset($item['tax_rate']) ? floatval($item['tax_rate']):0;//增值税
			$item['reg_rate']= isset($item['reg_rate']) ? floatval($item['reg_rate']):0;//消费税＝0
		}else{
			$item['postyun_code']="0";
			$item['postyun_rate']=0;
			$item['tax_rate']='0';
     ...
     * 获取单个商品关联的促销id
     * 
     * 新加一个SKU_ID参数，当sku_id 不为空时，item_id 取值来源于SKU——ID关联的数据item_id；
     */
    public function itemPromotionFirst($params,&$select_promotion= 0)  {
    	$activityList = array();
    	$db = app::get('sysuser')->database();
        $objMdlItemPromotionTag = app::get('sysitem')->model('item_promotion');
        $objMdlSku = app::get('sysitem')->model('sku');
        $cur_itemId = isset($params['item_id'])? intval($params['item_id']):false;
        if(isset($params["sku_id"]) && !empty($params["sku_id"]) && intval($params["sku_id"])>0){
        	$info  = $objMdlSku->getRow("sku_id,item_id,bn",array("sku_id"=> $params["sku_id"]));
        	if(isset($info["item_id"]) && intval($info["item_id"])>0){ $cur_itemId=intval($info["item_id"]);}
        }
        $prolist = $db->executeQuery('SELECT it.*,pr.rel_promotion_id,promotion_type,shop_id,promotion_name,start_time,end_time,check_status FROM `sysitem_item_promotion` it' 
        			.' LEFT JOIN `syspromotion_promotions` pr ON it.promotion_id = pr.`promotion_id` WHERE it.`item_id`=? ORDER BY start_time DESC',array($cur_itemId) )->fetchAll();
        if($prolist && !empty($prolist) &&  count($prolist)>0){
        	foreach($prolist as $k=> $act){
    			//活动有效性判断；
    		 	if($act && trim($act["check_status"])=="agree" && intval($act["start_time"]) >0 && intval($act["end_time"]) >0 &&  time() >= $act["start_time"] && time() < $act["end_time"] ){
    		 		$activityList[$act["promotion_id"]] = $act;
    		 	}
        	}
        	if($activityList && count($activityList )>0){
        		$temp = reset($activityList);
        		($temp && isset($temp["promotion_id"])) and $select_promotion = intval($temp["promotion_id"]);
        	}
        	return $activityList;
    	}
        return false;
     ...
     * 
     */
     function getItemCouponList($itemId,$shopid){
     	if(!$itemId || !$shopid ) {
     		throw new \LogicException(app::get('sysitem')->_('获取商品的参数异常'));
     		return false ;
     	}
     	$canuseCoupon = array();
     	//$itemId = 2057;
     	$this_today = time();//当前时间；
     	$db = app::get('sysuser')->database();
        $prolist = $db->fetchAll('SELECT cit.item_id ,c.* FROM  `syspromotion_coupon` c LEFT JOIN `syspromotion_coupon_item` cit  ON cit.`coupon_id` = c.`coupon_id`' 
        			.'  WHERE cit.`item_id` = ? AND shop_id = ? ORDER BY deduct_money DESC',array($itemId,$shopid));
        if($prolist && !empty($prolist) &&  count($prolist)>0){
        	foreach($prolist as $k=> $cop){
        		//判断优惠券是否可用；或是过期失效
        		$coupon_id  = isset($cop["coupon_id"]) ? intval($cop["coupon_id"]) :0 ;
        		$coupon_sttime  = isset($cop["canuse_start_time"]) ? intval($cop["canuse_start_time"]) :0 ;
        		$coupon_entime  = isset($cop["canuse_end_time"]) ? intval($cop["canuse_end_time"]) :0 ;
        		//领取时间
        		$coupon_send_s  = isset($cop["cansend_start_time"]) ? intval($cop["cansend_start_time"]) :0 ;
        		$coupon_send_e  = isset($cop["cansend_end_time"]) ? intval($cop["cansend_end_time"]) :0 ;
        		if($coupon_id && isset($cop["coupon_status"]) && trim($cop["coupon_status"]) =="agree"  && $this_today >= $coupon_sttime && $this_today< $coupon_entime) {
        			//判断是还在领取时间内
        			if(!($this_today >= $coupon_send_s && $this_today< $coupon_send_e)){
        				$cop["not_send"]= "暂停领取";
        			}
        			$canuseCoupon[$coupon_id]= $cop;  //一张有效的优惠券信息
        		}
        	}
        }
     	return  !empty($canuseCoupon) ? $canuseCoupon : false ; 
     }
