...
class topc_ctl_shopcenter extends topc_controller
{
    ...
    public function __construct($app)
    {
        parent::__construct();
        $this->app = $app;
        //$this->setLayoutFlag('shopcenter');  店铺首页暂时不装修，把默认的店铺内页面放出来。
    ...
    public function search()
    {
        ...
        $searchParams['fields'] = 'item_id,title,image_default_id,price,tax,sea_region';

        $itemsList = app::get('topc')->rpcCall('item.search',$searchParams);
        //print_r($itemsList);exit;
        //检测是否有参加团购活动
        if($itemsList['list'])
        {
    ...
    public function shopCouponList()
    {
       ...
       // 店铺优惠券信息,
        ...
        //返回数据结构＝＝＝ HTML
        if($showType && strtoupper($showType)=="HTML"){
        	$userCoupIds = array();
        	$params["user_id"] = userAuth::id();
        	$usercouponList= app::get('topc')->rpcCall('user.coupon.list', $params, 'buyer');//新加我领取过的券；
        	if(isset($usercouponList["coupons"]) &&$usercouponList["count"] >0 ){
        		foreach ($usercouponList["coupons"] as $k=> $kv){
        			if(isset($kv["coupon_id"]) && !empty($kv["coupon_id"]) && !empty($kv["coupon_code"])){
        				$userCoupIds[$kv["coupon_id"]] = $kv["coupon_code"];
        			}
        		}
        	}
        	$userCoupIds and $pagedata['userCoupids'] = count($userCoupIds) >0 ?$userCoupIds  : false ;//获取优惠券；
        	$pagedata['shopCouponCount'] = $couponListData['count']?intval($couponListData['count']) :0 ;//获取优惠券数量；
        	$msg = view::make('topc/cart/showcouponlist.html', $pagedata)->render();
        	return $this->splash('success',null,$msg,true);
        }
        $pagedata['file'] = "topc/shop/shopCouponList.html";
        return $this->page('topc/shop/shopCouponList.html', $pagedata);
    }
