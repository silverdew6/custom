...
class sysshop_data_shop{
    ...
    public function openShop($enterapplyId)
    {
        ...
        try
        {
			...
			$shopInfo['sea_region']    = $params['sea_region'];
			$shopInfo['tax']                = $params['tax'];
			$shopInfo['ebcCode']       = $params['ebcCode'];
			$shopInfo['ebcName']      = $params['ebcName'];
			$shopInfo['agentCode']    = $params['agentCode'];
			$shopInfo['agentName']   = $params['agentName'];
			$shopInfo['shortName']    = $params['shortName'];
            ...
            if(!$result)
            {
               ...
               $region = unserialize($shopInfo['sea_region']);
    ...
    public function getShopDetail($shopId,$row,$extends)
    {
        ...
        if($extends['info'])
        {
		 ...
		 $shopData['shop_info']['sea_region'] =  array_values(unserialize($shopData['shop_info']['sea_region']));
		     $ectoolsdata = app::get('ectools')->model('region');
             $date="";
             foreach($shopData['shop_info']['sea_region'] as $v){
              $filter = ['id' =>$v ];
	    	  $fields = "name";
		      $regiondate=$ectoolsdata ->getRow($fields, $filter);	
              $date = $date."     ".$regiondate['name'];
                  }
         $shopData['shop_info']['sea_region'] =$date;
		 $shopData['shop_info']['tax'] =  array_values(unserialize($shopData['shop_info']['tax']));
          $ectoolsdata = app::get('sysshop')->model('tax');

		       $tax="";
             foreach($shopData['shop_info']['tax'] as $v){
              $filter = ['id' =>$v ];
	    	  $fields = "name";
		      $regiondate=$ectoolsdata ->getRow($fields, $filter);	
              $tax = $tax."     ".$regiondate['name'];
                  }
			$shopData['shop_info']['tax'] =$tax;
