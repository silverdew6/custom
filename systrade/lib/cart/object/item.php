    ...
    public function processCartObject($row,$itemsData,$skusData)
    {
        ...
        if($activityDetail['activity_price']>0)
        {
			...
			if($objectData['tax']>1){ //保税和直邮
				...
				//计算税；
				$objectData['price']['zonghe_rate_price'] = $zh_rate>0 ? round( $skusData[$skuId]['price'] * $zh_rate,3) :0 ; //综合税
				$objectData['price']['zonghe_rate_total'] = isset($row['quantity']) ? round( $skusData[$skuId]['price'] * $zh_rate,3) * $row['quantity'] :0 ; //综合税
				//综合税率
