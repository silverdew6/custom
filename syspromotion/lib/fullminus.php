...
class syspromotion_fullminus extends syspromotion_abstract_promotions {
    ...
    public function deleteFullminus($params)
    {
        ...
        if( time() > $fullminusInfo['start_time']&& time() < $fullminusInfo['end_time'] )
        {
    ...
    private function __saveFullminusItem(&$fullminusData, $promotion_id)
    {
        ...
        // 先删除满减关联的商品标签内容；
        app::get('syspromotion')->rpcCall('item.promotion.deleteTag', array("promotion_id"=> $promotion_id));
        foreach($fullminusData['rel_item_ids'] as $itemid)
        {
            ...
            $apiData = array( 'item_id' => $itemid, 'promotion_id' => $promotion_id);
            // 新的商品及促销关联接口 // 给商品的活动信息添加关联；
    ...
    private function __preareData($data) {
        $aResult = array();
        $aResult = $data;
        $can_edit_mink = false ; //管理员可以    强行修改 ； 其它人不可以

        $objMdlFullminus = app::get('syspromotion')->model('fullminus');
        if($data['fullminus_id']){
            $fullminusInfo = $objMdlFullminus->getRow('*', array('fullminus_id'=>$data['fullminus_id']));
            if(!$can_edit_mink && time() >= $fullminusInfo['start_time'] && time() < $fullminusInfo['end_time'] )
            {
        ...
        //参加活动的产品ID
        $aResult['rel_item_ids'] = explode(',', $data['fullminus_rel_itemids']);//选择的产品ID
        if(!$this->checkPromItemRegion($aResult['rel_item_ids'])){
        	 throw new \LogicException('抱歉，暂不支持不同业务类型或不同仓库的商品!请检查');
        }
        $objMdlFullminusItem = app::get('syspromotion')->model('fullminus_item');
