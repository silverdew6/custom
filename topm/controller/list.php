   ...
   private function __getItems($postdata)
    {
        ...
        //如果不是从分类进入，并且没有关键字搜索则不能进入列表页 ===> 新增按品牌来搜索所有品牌下的产品 by xch
        $params['search_keywords'] = trim($params['search_keywords']);
        if( empty($params['cat_id']) && empty($params['search_keywords']) && empty($params['brand_id']) )
        {
        	if(isset($params['tax']) && intval($params['tax'])>0){  //新增按完税和直邮来做；
        		$params['tax'] = intval($params['tax']);
        	}else{
        		 return false;
        	}
        }
    ...
    public function ajaxItemShow()
    {
	  ...
	  //2016-04-19 以下为手工添加的活动页面
	public function qingrenjie2016_m()
  {
	return $this->page('topm/list/qingrenjie2016.html');
  }
  
  	public function pingshi_m()
  {
	return $this->page('topm/list/pingshi_m.html');
  }
  public function muying_m()
  {
	return $this->page('topm/list/muying_m.html');
  }
  public function shenghuo_m()
  {
	return $this->page('topm/list/shenghuo_m.html');
  }
  public function lingshi_m()
  {
	return $this->page('topm/list/lingshi_m.html');
  }
  public function meizhuang_m()
  {
	return $this->page('topm/list/meizhuang_m.html');
  }
  public function qingshe_m()
  {
	return $this->page('topm/list/qingshe_m.html');
  }
}
