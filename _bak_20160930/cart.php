    ...
    public function total()
    {
        ...
        foreach ($cartInfo['resultCartData'] as $shop_id => $tval) {
            ...
            if(intval($number)>0 && floatval($current_postfee)>0 ){
            	$totalInfo["payment"]=$totalInfo["total_fee"] + ( $current_postfee * $number );
            }
            ...
            //拆单，拆成多少个单，就需要有几个快递费用；
            $trade_data['shop'][$shop_id]['child_post_num'] = intval($number)>0 ? intval($number) :1;
