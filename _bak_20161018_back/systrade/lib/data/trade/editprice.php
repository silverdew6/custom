...
class systrade_data_trade_editprice
{
    ...
    public function generate($params)
    {
		...
		//throw new \LogicException("DDD".print_r($aDataTmp,false));
        $db = app::get('systrade')->database();
        $transaction_status = $db->beginTransaction();
        try
        {
        	$params["selfLog"] = json_encode($aDataTmp); 	//操作参数记录
            if(!app::get('systrade')->model('trade')->save($aDataTmp))
            {
                throw new \LogicException("取消订单失败，更新数据库失败");
            }
            $this->addLog($params['tid'], $params); //添加操作日志
            $db->commit();
        }
        catch (Exception $e)
        {
    ...
    private function addLog(&$tid, &$params)
    {
        ...
        $sdfTradeLog = array(
            ...
            'behavior' => 'update',
            'log_text' => $logText,
            'log_param'=> isset($params['selfLog'])? $params['selfLog'] : ""
        );
		//throw new \LogicException("DDD".print_r($sdfTradeLog,false));
