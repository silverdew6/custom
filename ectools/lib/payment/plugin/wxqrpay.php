...
//    echo '<pre>';print_r($this->fields); exit;   
        ...
        $is_wap_flag =  false;
        $up_uri =   isset($_SERVER['HTTP_REFERER']) ? trim($_SERVER['HTTP_REFERER']) : "";
        $is_ww = trim($up_uri)!="" ? strrpos($up_uri,"/wap/"):false;
        if($is_ww!== false ){
        	$is_wap_flag = true;//来源手机端
        }
        ...
        $strHtml .="<meta charset='utf-8'><title>微信扫码支付</title><meta name='viewport' content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no'/>";
        $strHtml .= "</head>";
        if($is_wap_flag){
        	$strHtml .= '<body><div style="width: 100%;max-width:500px;">';
        }else{
        	$strHtml .= '<body><div style="width: 100%;max-width:850px;">';
        }
        if( empty($this->fields['code_url']) ){
	        ...
	        $strHtml .=
	            '<div id="qrcode">
		            <div class="wrapper">';
		    $strHtml .= $is_wap_flag ? '<div style="float:left; padding:0px;width:100%;">': '<div style="float:left; padding:1px;">' ;     	
		    $strHtml .= '<div style="font-size:1.8em;color:red;width:100%;margin:0.5em auto;height:1.5em;text-align:center;">微信扫码支付</div>
		                <img src="'.$qrcode_url.'" alt="微信扫码支付" style="border: 1px solid #efefef;max-width: 100%;"/>
		                <div style="background:#ef7772; text-align:center; font-size:1.03em;padding:8px; color: #fff">请用微信扫一扫进行支付<br>移动端支付时，请先将二维码保存下来再扫描</div>
		              </div>';
		     $strHtml .= $is_wap_flag ? '': '<div style="margin-top: 5em;float:left;display:inline-block;"><img src="'.$image.'" alt=""></div>';
		     $strHtml .= '<button onclick="history.go(-1)" style="' .($is_wap_flag ? 'position: fixed;':'').
		     		'bottom:1em;right:0.5em;height:1.8em;font-size:1.2em;padding:2px 8px;border:1px solid #DDD;color:green;border-radius: 4px;margin:1em auto;width: 4em;font-weight: bold;">返回</button>
		        	</div></div>';
        }
        $strHtml .= '</div>';
        $strHtml .= '</body><script>
