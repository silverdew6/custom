...
class sysshop_ctl_admin_enterapply extends desktop_controller{
    ...
    public function doExamine($enterapplyId)
    {
        ...
        $pagedata['itemdata'] = $list;
		 $pagedata['applydata']['sea_region'] =  array_values(unserialize($list['sea_region']));
		     $ectoolsdata = app::get('ectools')->model('region');
             $date="";
             foreach($pagedata['applydata']['sea_region'] as $v){
              $filter = ['id' =>$v ];
	    	  $fields = "name";
		      $regiondate=$ectoolsdata ->getRow($fields, $filter);	
              $date = $date."     ".$regiondate['name'];
                  }
         $pagedata['itemdata']['sea_region'] =$date;
		 $pagedata['itemdata']['tax'] =  array_values(unserialize($list['tax']));
       if (in_array("2",$pagedata['itemdata']['tax'] , TRUE)||in_array("3",$pagedata['itemdata']['tax'] , TRUE))
		{
		   $pagedata['itemdata']['is_tax'] =1;
		}
          $ectoolsdata = app::get('sysshop')->model('tax');

		       $tax="";
             foreach($pagedata['itemdata']['tax'] as $v){
              $filter = ['id' =>$v ];
	    	  $fields = "name";
		      $regiondate=$ectoolsdata ->getRow($fields, $filter);	
              $tax = $tax."     ".$regiondate['name'];
                  }
			$pagedata['itemdata']['tax'] =$tax;
