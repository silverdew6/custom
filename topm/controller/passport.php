...
class topm_ctl_passport extends topm_controller
{
    ...
    function _red_weixin( $next_url = ""){
        ...
        $wxsetting = $trust->getSetting();//return $wxsetting;
        ...
        $appid = 'wxd5e1e4e198f0e216';//新增
        $state = isset($next_url) && trim($next_url)!="" ? trim($next_url) :""; //返回URL
		$goHosturl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$appid}&redirect_uri={$redirect_uri}&response_type=code&scope={$scope}&state={$state}#wechat_redirect";//return $goHosturl;
