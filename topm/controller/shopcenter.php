...
class topm_ctl_shopcenter extends topm_controller
{
    ...
    public function shopCouponList()
    {
        ...
        if($showType && strtoupper($showType)=="HTML"){
        	...
        	$msg = view::make('topm/cart/showcouponlist.html', $pagedata)->render();
    ...
    public function getCouponCode()
    {
        ...
        try
        {
            ...
            $success_url = url::action('topm_ctl_shopcenter@getCouponResult', array('coupon_id'=>$coupon_id, 'shop_id'=>$shopId));
            $apiData = array(
                 'coupon_id' => $coupon_id,
                 'user_id' =>$user_id,
                 'shop_id' =>$shopId,
            ...
            //Debug code ==  return $this->splash('success',$success_url ,'领取优惠券成功!' , true);
            //领取优惠券
            $result = app::get('topm')->rpcCall('user.coupon.getCode', $apiData);
            if(isset($result) && !empty($result) && trim($result["coupon_code"]) != "") {
                //返回的数据            
                return $this->splash('success', $success_url, '领取优惠券成功', true);
            }else{
