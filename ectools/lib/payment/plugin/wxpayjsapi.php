...
final class ectools_payment_plugin_wxpayjsapi extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function dopay($payment)
    {
        ...
        //echo $this->get_html();exit;//调起微信支付控件
//        $res = $this->dq($payment['payment_id']);
//        $resArr[0] = $this->fields['jsApiParameters'];
//        $resArr[1] = $res;
//        var_dump($resArr) ;exit;
    }
    //调起微信支付控件
    public function dq($payment_id){
        ...
        $project = $this->getSignPackage();return $project;
        $StrHtml = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title></title></head><body></body><script src="http://res.wx.qq.com/open/js/jweixin-1.0.0.js"></script><script>';
        $StrHtml .= 'wx.config({
    debug: true,
    appId: '.$project['appId'].',
    timestamp: '.$project['timestamp'].',
    nonceStr:'.$project['nonceStr'].',
    signature: '.$project['signature'].',
    jsApiList: ["chooseWXPay"]
  });';
   $StrHtml .= 'wx.ready(function () {
            alert(123);
            // 在这里调用 API
        });';
        $StrHtml .= '</script></html>';
        $html = "<!DOCTYPE html><html lang='en'><head><meta charset='utf-8'><title>微信安全支付</title></head><body><span id='pay' appId = ".$project['appId']."  timestamp=".$project['timestamp']." nonceStr=".$project['nonceStr']." signature=".$project['signature']."></span></body><script src='http://res.wx.qq.com/open/js/jweixin-1.0.0.js'></script><script type='text/javascript'>wx.config({
    debug:true,
    appId:$('#pay').attr('appId'),
    timeStamp:$('#pay').attr('timeStamp'),
    nonceStr:$('#pay').attr('nonceStr'),
    signature:$('#pay').attr('signature'),
    jsApiList:['chooseWXPay']
});wx.ready(function(){alert(2);});wx.error(function(res){alert('验证失败');});</script></html>";
        return $StrHtml;
    }
    public function getSignPackage() {
        ...
        $string = "jsapi_ticket=$jsapiTicket&noncestr=$nonceStr&timeStamp=$timestamp&url=$url?params=value";
    ...
    private function getAccessToken() {
        ...
        if($this->atime+7000<time()){
            $this->atime = time();
            $appid      = trim($this->getConf('appId',    __CLASS__));
            $secret      = trim($this->getConf('Appsecret',    __CLASS__));
            $url="https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$appid}&secret={$secret}";
            $json = $this->https_request($url);//返回一个json值
            $arr = json_decode($json,true);//json->array
            $this->atoken = $arr['access_token'];//返回token
        }
