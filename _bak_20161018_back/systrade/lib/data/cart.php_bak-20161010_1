 ...
 *重新整合新数组，分保税，完税，直邮。
 */
		private function __taxShopCartInfo($shopObjectData,$shopId)
	{
        //2016/3/9lcd  开始拆单
       $newshopData = [];
        foreach( $shopObjectData as $row)
        {
            $newshopData[$row['tax_sea_region']][] = $row;

		   //获取业务模式和区域
            $tax      =substr($row['tax_sea_region'],0,1);
			$region=substr($row['tax_sea_region'], 1);
            $filters['tax_id']=$shopId.$row['tax_sea_region'];
			$filters['user_id']=userAuth::id();
		
			$select_id =     $this->objCarttax->getRow('select_id,disabled_id',$filters);
			if($select_id['select_id']){
				$newshopData[$row['tax_sea_region']]['select_id'] = $select_id['select_id'];
			}else{
				$newshopData[$row['tax_sea_region']]['select_id'] = 0;
			}
			if($select_id['disabled_id']){
				$newshopData[$row['tax_sea_region']]['disabled_id'] = $select_id['disabled_id'];
			}else{
				$newshopData[$row['tax_sea_region']]['disabled_id'] = 0;
			}
           $newshopData[$row['tax_sea_region']]['tax'] = $tax;//业务模式
		    $newshopData[$row['tax_sea_region']]['sea_region'] = $region;//业务区域
        }
		ksort($newshopData);
		return $newshopData;
	}
    ...
    private function __preCartInfo($cartData, $needInvalid, $platform)
    {
        ...
        foreach( $newCartData as $shopId=>$shopCartData )
        {
            ...
            if( $shopObjectData )
            {
				...
				$resultCartData[$shopId]['object2'] = $this->__taxShopCartInfo($shopObjectData,$shopId);//开始拆单
