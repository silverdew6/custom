...
class syspromotion_solutions_fullminus implements syspromotion_interface_promotions{
    ...
    public function apply($params)
    {
        ...
        $param3 =array('promotion_id'=>$params['promotion_id'],'user_id'=> intval($params["user_id"]) ) ;
        $applyNum = app::get('syspromotion')->rpcCall('trade.promotion.applynum', $param3); //统计当前活动已参加的申请次数；
 		/*if($params['fullminus_id']==2) 
        {
        	print_r($fullminusInfo);
        	var_dump($applyNum);
        }*/
        if($applyNum>=$fullminusInfo['join_limit'] )
        {
            return 0;  // throw new \LogicException('可参与的满减次数已经用完!');
        }
        ...
        {
            return 0;  // throw new \LogicException('您的当前会员等级不可参加此促销!');
        }
        // 满减金额的规则检验
        ...
        if($params['forPromotionTotalPrice'] >=$ruleArray['full'][$ruleLength-1] ) {
            if($fullminusInfo['canjoin_repeat']=='1'){ //上不封顶操作；
