...
class topm_ctl_member extends topm_controller {
    ...
    {
        if(isset($_SESSION["offer_coupon_sendsuccess"]) && $_SESSION["offer_coupon_sendsuccess"] == true ){
    		$pagedata["offer_coupon_sendsuccess"] = true; //是否显示有领取到的优惠券
	...
	//线下券数量
 	$filtercps = array("user_id"=> $userId, "shop_id"=>100000);
        $offen_model  = app::get('sysuser')->model('cpoffen');
        $offen_cps = $offen_model->count($filtercps);
        if(isset($offen_cps) && intval($offen_cps)>0 ) {
        	$pagedata['coupon']["count"]=  intval($pagedata['coupon']["count"]) + intval($offen_cps);
        }
        $pagedata['title'] = "会员中心";
    ...
    /*public function jsapi(){
        $ObjData = input::get();
        $return_url = url::action('topm_ctl_paycenter@finish', array('payment_id'=>$ObjData['payment_id']));
        $pagedata['url'] = $return_url;
        $pagedata['data'] = $ObjData['data'];
        return $this->page('topm/member/jsapi.html',$pagedata);
    }*/
