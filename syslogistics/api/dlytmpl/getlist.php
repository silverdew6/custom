...
class syslogistics_api_dlytmpl_getlist{
    ...
    public function getList($params)
    {
        if($params['shop_cart'])
        {
            $filter['shop_cart'] = $params['shop_cart'] ;
        }

        if($params['port'])
        {
            $filter['port'] = $params['port'] ;
        }
...
//添加模式
        if($params['select_tax'])
        {
            $filter['select_tax'] = $params['select_tax'] ;
        }
//添加区域
		        if($params['select_region'])
        {
            $filter['select_region'] = $params['select_region'] ;
        }
         //分页使用
