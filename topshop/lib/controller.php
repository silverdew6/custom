...
class topshop_controller extends base_routing_controller
{
    ...
    public function page($view, $pagedata = array())
    {
        ...
        //SEO 默认相关配置参数；
        $topshopPageParams['seo']= array("title"=> "精茂商家系统","keywords"=>"精茂城,商家系统,精茂电商平台","description"=>"精茂电商平台是一套跨境进口平台。");
        if( $this->contentHeaderTitle )
        {
            $topshopPageParams['contentTitle'] = $this->contentHeaderTitle;
            $topshopPageParams['seo']["title"] =  $this->contentHeaderTitle ." - " .$topshopPageParams['seo']["title"] ;
        }
        ...
        //获取logo
        $logo = app::get('sysconf')->getConf('site.logo');
        $pagedata['logo'] = $logo;
