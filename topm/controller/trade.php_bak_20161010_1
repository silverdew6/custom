    ...
    public function create()
    {
        ...
        if($cartInfo){
        	foreach($cartInfo as $kx=> $onecitem){
        		if($onecitem && isset($onecitem["tax"]) && intval($onecitem["tax"])==3){
		...
		//2016/4/5 雷成德 ————〉新加直邮才需要使用身份证号 by XCH 20160923
		if(!$is_tax_flag){ //不是完税，需要身份证号
			if(!$addr['card_id'])
	        {
	            //重新编辑身份证号 http://www.myjmall.com/wap/cart-editaddr.html?addr_id=279
	            $msg = app::get('topm')->_("收件人身份证号不能为空！");
	            $goEdurl =url::action('topm_ctl_cart@editAddr',array('addr_id'=>$postData['addr_id'],'idcard'=>1));
	            return $this->splash('error', $goEdurl, $msg, true);
	        }
		}
