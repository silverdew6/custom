...
class systrade_api_trade_updatePrice{
    ...
    {
    	if(isset($params['oauth_json']) && !empty($params['oauth_json'])){
    		$hander = (array)json_decode($params['oauth_json']);
    		$hander and $params['oauth']= $hander;//把json对象数据给值 oauth;
    	}
        ...
        {
            $params['operator']['op_id'] = $params['oauth']['account_id'];
            $params['operator']['op_account'] = $params['oauth']['account_name'];
            $params['operator']['account_type'] = $params['oauth']['auth_type'];
        }
        try{
